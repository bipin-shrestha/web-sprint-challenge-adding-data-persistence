...
- Relational Database are database where the data are stored in tabular format. Basically it's clean, organised way of keeping the data in the rows and colums whereas SQL is just a standard language which used to access and control the database.
2. Why do tables need a Primary Key?
-  Primary key is used to implement a relationship between two tables and as primary keys are unique it makes easier to maintain the data.
3. What is the name given to a table column that references the Primary Key on another table?
- Foreign key.
4. What do we need in order to have a _many to many_ relationship between two tables?
- Many to many realationship usually occurs when many records in the table are linked with many records in the another table. In many to many relationship we need to use associative table.
